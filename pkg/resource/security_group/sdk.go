...
func (rm *resourceManager) sdkFind(
	...
	resp, respErr := rm.sdkapi.DescribeSecurityGroupsWithContext(ctx, input)
	rm.metrics.RecordAPICall("READ_MANY", "DescribeSecurityGroups", respErr)
	if respErr != nil {
...
func (rm *resourceManager) sdkCreate(
	...
	resp, respErr := rm.sdkapi.CreateSecurityGroupWithContext(ctx, input)
	rm.metrics.RecordAPICall("CREATE", "CreateSecurityGroup", respErr)
	if respErr != nil {
		return nil, respErr
	}
	// Merge in the information we read from the API call above to the copy of
	...
	if resp.GroupId != nil {
		ko.Status.GroupID = resp.GroupId
	}
	if resp.Tags != nil {
		f1 := []*svcapitypes.Tag{}
...
func (rm *resourceManager) sdkDelete(
	...
	_, respErr := rm.sdkapi.DeleteSecurityGroupWithContext(ctx, input)
	rm.metrics.RecordAPICall("DELETE", "DeleteSecurityGroup", respErr)
	return respErr
}
...
func (rm *resourceManager) newDeleteRequestPayload(
	...
	if r.ko.Status.GroupID != nil {
		res.SetGroupId(*r.ko.Status.GroupID)
	}
	if r.ko.Spec.GroupName != nil {
		res.SetGroupName(*r.ko.Spec.GroupName)
	}
